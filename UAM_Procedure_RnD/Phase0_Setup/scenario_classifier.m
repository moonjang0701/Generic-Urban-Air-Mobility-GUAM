...
if exist(ds4_path, 'file')
    ds4 = load(ds4_path);
    % Data_Set_4 contains arrays, check if any failure is defined
    has_surf_fail = isfield(ds4, 'Surf_FailInit_Array');
    has_prop_fail = isfield(ds4, 'Prop_FailInit_Array');
    if has_surf_fail || has_prop_fail
        fprintf('    ✓ Loaded failure scenario data\n');
    else
        fprintf('    ⚠ No failure data found in Data_Set_4\n');
    end
else
...
for i = 1:n_runs
    ...
    % Data_Set_1.own_traj is a cell array: each row = 1 scenario
    % Columns: 1=wptsX, 2=wptsY, 3=wptsZ, 4=time_wptsX, 5=time_wptsY, 6=time_wptsZ
    if i <= size(ds1.own_traj, 1)
        wptsX_cell = ds1.own_traj{i, 1};
        wptsY_cell = ds1.own_traj{i, 2};
        wptsZ_cell = ds1.own_traj{i, 3};
        time_wptsX_cell = ds1.own_traj{i, 4};
        time_wptsY_cell = ds1.own_traj{i, 5};
        time_wptsZ_cell = ds1.own_traj{i, 6};
        
        % Extract actual data from cells
        if ~isempty(wptsX_cell) && ~isempty(wptsY_cell) && ~isempty(wptsZ_cell)
            wptsX = wptsX_cell;
            wptsY = wptsY_cell;
            wptsZ = wptsZ_cell;
        ...
        if max(abs(dheading)) > 0
            ...
            % Duration estimate (from time waypoints)
            if ~isempty(time_wptsX_cell)
                duration = time_wptsX_cell(end);
            else
                duration = 0;
            end
        ...
        % No trajectory data for this index
        scenario_catalog.runs(i).trajectory_type = 'unknown';
        scenario_catalog.runs(i).vertical_profile = 'unknown';
        scenario_catalog.runs(i).procedure_style = 'unknown';
        scenario_catalog.runs(i).max_bank_angle = 0;
    ...
    % Check for failures
    % Data_Set_4 structure: columns are scenarios, check if any failure is active
    if ~isempty(ds4) && isstruct(ds4)
        has_failure = false;
        if isfield(ds4, 'Surf_FailInit_Array') && i <= size(ds4.Surf_FailInit_Array, 2)
            % Check if any surface failure is non-zero
            if any(ds4.Surf_FailInit_Array(:, i) ~= 0)
                has_failure = true;
            end
        end
        if isfield(ds4, 'Prop_FailInit_Array') && i <= size(ds4.Prop_FailInit_Array, 2)
            % Check if any propulsor failure is non-zero
