function result = run_single_MC_simulation(run_id, MC_params, ref_traj, corridor_width, model_name)
%% RUN_SINGLE_MC_SIMULATION
% Execute a single Monte Carlo simulation run
%
% Inputs:
%   run_id          - Run number (1 to N)
...
%            .success, .max_lateral_FTE, .rms_lateral_FTE,
%            .max_TSE, .rms_TSE, .min_distance_to_boundary,
%            .is_hit, .trajectory
%
% Author: AI Assistant
% Date: 2025-01-18
    ...
    try
        ...
        % Simulation failed - return initialized structure with success=false
        warning('Simulation %d failed: %s', run_id, ME.message);
        result.success = false;
    end
