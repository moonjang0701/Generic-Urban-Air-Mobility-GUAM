%% Monte Carlo TSE Safety Assessment for UAM Corridor - QUICK TEST VERSION
% =====================================================
% Purpose: QUICK TEST with only 10 samples (~10-15 minutes)
%          Fast verification before running full 500-sample analysis
%
% Author: AI Assistant
% Date: 2025-01-18
%
% ⚡ QUICK TEST MODE: N = 10 samples
% 
% For full analysis (N=500), use: run_MC_TSE_safety.m
%
% =====================================================
...
fprintf('╔══════════════════════════════════════════════════════════════╗\n');
fprintf('║  Monte Carlo TSE Safety Assessment - QUICK TEST (N=10)      ║\n');
...
N_MONTE_CARLO = 10;             % ⚡ QUICK TEST: Only 10 samples (~10-15 min)
USE_PARALLEL = false;           % Set true for parfor speedup
RANDOM_SEED = 42;               % For reproducibility

fprintf('⚡⚡⚡ QUICK TEST MODE: N = %d samples ⚡⚡⚡\n', N_MONTE_CARLO);
fprintf('For full analysis (N=500), use: run_MC_TSE_safety.m\n\n');
...
if USE_PARALLEL
    ...
    for idx = 1:N_MONTE_CARLO
        ...
        fprintf('Run %d/%d: ', idx, N_MONTE_CARLO);
        ...
        if ~isstruct(MC_results_i)
            error('Run %d: MC_results_i is not a struct!', idx);
        end
        
        % Debug: check required fields
        required_fields = {'max_lateral_FTE', 'success'};
        for f = 1:length(required_fields)
            if ~isfield(MC_results_i, required_fields{f})
                error('Run %d: Missing field %s', idx, required_fields{f});
            end
        end
        
        fprintf('success=%d, FTE=%.2f ', MC_results_i.success, MC_results_i.max_lateral_FTE);
